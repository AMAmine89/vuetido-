    ...
    <v-dialog v-model="dialog">
      <v-card>
        <v-card-title
          class="headline"
          v-text="
            addOrEdit === 'add' ? 'Add Task' : 'Edit ' + task_field.task_name
          "
        ></v-card-title>

        <v-card-text>
          <v-text-field
            v-model="task_field.task_name"
            label="Task Name"
            :rules="taskRules"
          ></v-text-field>
          <v-select
...
export default {
  ...
  methods: {
    ...
    checkValues() {
      ...
      return !(
        this.task_field.task_name &&
        this.task_field.asignTo &&
        this.task_field.importance
      );
    }
  }
};
