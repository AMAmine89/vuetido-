  ...
  <div>
    <v-container>
      <v-row class="justify-center">
        <v-list class="tasks-list">
          ...
          >
            <div :class="task[0].stat === 'Traitment' ? '' : 'text--disabled'">
              {{ task[0].task_name }}
            </div>
            <div>
              <v-chip :color="chipColor(task[0].importance)" class="mr-3">
                {{ task[0].importance }}
              </v-chip>
    ...
    <v-dialog v-model="dialog" max-width="290">
      <v-card>
        <v-card-title
          class="headline"
          v-text="'Remove ' + task_field.task_name"
        ></v-card-title>
        ...
        <v-card-actions>
          ...
          <v-btn color="red" text @click="remove(task_field)">Remove</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <notification
